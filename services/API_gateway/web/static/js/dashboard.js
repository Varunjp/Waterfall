(() => {
  ...
  function fmt(dateStr) {
    ...
    const tz = timeZoneLabel(d);
    return tz ? `${text} ${tz}` : text;
  }

  function toLocalInputValue(dateStr, includeTime = false) {
    const d = parseDate(dateStr);
    if (!d) return '';
  ...
  function toUTCDateTimeValue(dateStr) {
    if (!dateStr) return null;
  ...
  async function loadJobs(page = 0, filter = currentFilter, startDate = currentStartDate, endDate = currentEndDate) {
    ...
    if (startDate) startEl.value = toLocalInputValue(startDate);
    if (endDate)   endEl.value   = toLocalInputValue(endDate);
    ...
    document.getElementById('btnApply').addEventListener('click', () => {
      loadJobs(0, currentFilter, toUTCDateBoundary(startEl.value), toUTCDateBoundary(endEl.value, true));
    });
  ...
  function openModal(job) {
    ...
    if (isPending) {
      ...
      // Pre-fill using local browser time while keeping UTC as the API format.
      const scheduleEl = document.getElementById('edit-schedule');
      if (job.scheduleAt) {
        const localVal = toLocalInputValue(job.scheduleAt, true);
        scheduleEl.value = localVal;
        scheduleEl.dataset.original = localVal;
      } else {
        scheduleEl.value = '';
  ...
  async function updateJob(jobId) {
    ...
    if (scheduleVal && scheduleChanged) {
      const scheduleISO = toUTCDateTimeValue(scheduleVal);
      if (!scheduleISO) {
        errEl.textContent = 'Schedule time is invalid.';
        return;
      }
      body.schedule_at = scheduleISO;
    }
  ...
  function renderSubscription(d) {
    ...
    // For FREE plans: use FreeLimit/FreeUsage for the usage bar; PlanLimit shown as 0
    const used       = isFree ? (d.FreeUsage  ?? 0) : (d.CurrentLimit ?? 0);
    const limit      = isFree ? (d.FreeLimit  ?? 1) : (d.PlanLimit    ?? 1);
    const pct        = Math.min(100, Math.round((used / (limit || 1)) * 100));

    const planLimitDisplay = isFree ? 0 : (d.PlanLimit ?? 0);
    ...
    // Free-tier extra info block (only on FREE plan)
    const freeInfoBlock = isFree ? `
        <!-- ── Free tier note ── -->
        <div class="sub-card" style="border-color:rgba(168,212,240,0.2);background:rgba(168,212,240,0.04)">
          <p class="sub-card-label" style="color:var(--status-running)">Free Tier Allocation</p>
          <div style="display:flex;align-items:baseline;gap:8px;margin-top:6px">
            <span style="font-family:'Bebas Neue',sans-serif;font-size:32px;letter-spacing:0.04em;color:var(--status-running)">${(d.FreeUsage ?? 0).toLocaleString()}</span>
            <span style="color:var(--gray-dim);font-size:18px">/</span>
            <span style="font-size:13px;font-weight:300;color:var(--gray-mid)">${(d.FreeLimit ?? 0).toLocaleString()} free jobs</span>
          </div>
          <p style="font-size:11px;letter-spacing:0.06em;color:var(--status-running);margin-top:8px">
            ${(d.FreeLimit ?? 0) - (d.FreeUsage ?? 0) > 0
              ? `${((d.FreeLimit ?? 0) - (d.FreeUsage ?? 0)).toLocaleString()} free jobs remaining`
              : 'Free job quota exhausted'}
          </p>
        </div>` : '';
      ...
      <div class="sub-layout">
        ...
        <div class="sub-card sub-usage-card">
          <div class="sub-usage-header">
            <div>
              <p class="sub-card-label">${isFree ? 'Free Tier Usage' : 'Monthly Usage'}</p>
        ...
        <div class="sub-meta-grid">
          <div class="sub-meta-cell">
            ...
            <p class="sub-meta-val">${planLimitDisplay.toLocaleString()} <span>jobs / mo</span></p>
          </div>
          <div class="sub-meta-cell">
          ...
          <div class="sub-meta-cell">
            <p class="sub-card-label">Free Limit</p>
            <p class="sub-meta-val" style="color:var(--status-running)">${(d.FreeLimit ?? 0).toLocaleString()}</p>
          </div>
          <div class="sub-meta-cell">
            <p class="sub-card-label">Free Usage</p>
            <p class="sub-meta-val" style="color:var(--status-running)">${(d.FreeUsage ?? 0).toLocaleString()}</p>
          </div>
          <div class="sub-meta-cell">
